...
module.exports = {
  ...
  toJson: function (src, dest, config) {
    ...
    let idx0 = src.lastIndexOf("/")
    let idx1 = src.lastIndexOf(".");
    let excelName = src.substring(idx0 + 1, idx1);

    parseWorkbook(workbook, dest, headIndex, uglifyJson, excelName,config);
  }
};
 ...
 * @param       {[Number]} headIndex     index of head line.
 */
function parseWorkbook(workbook, dest, headIndex, uglifyJson, excelName,config) {
  ...
  workbook.forEach(sheet => {
    ...
    if (sheet.name.indexOf('@') === -1 && sheet.name[0] !== "#" && !isOver) {
      let parsedSheet = parseSheet(sheet, headIndex,config);
      var targetNme = sheet.name.split('|');
      var sheetName = targetNme[1].toLowerCase().trim();
      let dest_file = path.resolve(dest, sheetName + ".json");
      let resultJson = JSON.stringify(parsedSheet.result, null, uglifyJson ? 0 : 2); //, null, 2
      ...
      dtsstring += formatDTS(sheetName, parsedSheet.head);
    }
    // isOver = true;
  });

  console.log("d.ts file", dest, excelName);
...
function formatDTS(name, head) {
  ...
  strHead = "interface " + className + "Template {\r\n";
 ...
 * @private
 */
function parseSheet(sheet, headIndex,config) {

  console.log('\t parsing sheet', sheet.name);

  if (sheet && sheet.data) {

    let head = parseHead(sheet, headIndex,config);
...
function parseHead(sheet, headIndex,config) {

  let typeRow = sheet.data[config.xlsx.type - 1];
  let headRow = sheet.data[config.xlsx.head - 1];
  ...
  let head = {
    ...
    getIdKey: function () {
      let id_col_index = this.types.indexOf(DataType.SN);
      if (id_col_index < 0) {
        throw '获取不到id列的名字';
      }
  ...
  headRow.forEach(cell => {
    ...
    if(name === DataType.SN){
      console.log("map 表")
       ...
       head.types.push(name);
    }
    head.names.push(name);
  });
  let index = 0;
  typeRow.forEach(cell => {
    if(index == 0 && head.sheetType == SheetType.PRIMARY){

    }else{
      let type = DataType.UNKOWN;
      type = cell.toLowerCase().trim();
      head.types.push(type);
    }
    index++;
  });
  console.log("这是什么鬼",head)
  return head;
}
